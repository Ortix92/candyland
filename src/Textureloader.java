...
public class Textureloader {
	...
	public static void load() {
		...
		try {
			...
			BufferedImage CandyWallImage = ImageIO.read(new File(
					"src/CandyWall.png"));
			...
			Stuiterbal = TextureIO.newTexture(StuiterbalImage, false);
	...
	public static void poptart(GL gl, float size) {
		...
		if (loadtextures) {
			PoptartTexture.disable(); // disable texture so that next objects
										// won't have it.
		}
	}
	...
	public static void stuiterbal(float size, int roundness) {
		// makes the head.
		if (loadtextures) {
			if (!load) { // checks if loading is needed
				load(); // loads textures
		...
		GLU glu = new GLU(); // needed to make a sphere with texture coordinates
		GLUquadric stuiterbal = glu.gluNewQuadric(); // makes a quadric object
		glu.gluQuadricTexture(stuiterbal, true); // Makes the quadric object head
											// capable of having textures.
		glu.gluSphere(stuiterbal, size, roundness, roundness); // make the
																// object into a
																// sphere
